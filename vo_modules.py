...
class VisualOdometry():
    ...
    def deep_flow_forward(self, in_cur_data, in_ref_data, forward_backward):
        ...
        print(np.sort(ref_data['depth'][ref_id].flatten()))
        depth_range_mask = (ref_data['depth'][ref_id] < self.cfg.depth.min_depth) + (ref_data['depth'][ref_id] > self.cfg.depth.max_depth)
        mask = depth_range_mask + cur_data['mask'] + ref_data['mask'][ref_data['id'][0]]
    ...
    def main(self):
        ...
        for img_id in tqdm(range(start_frame, len_seq)):
            ...
            # Reading mask
            mask = np.load(self.mask_dir+"/{:06d}.npy".format(img_id))
